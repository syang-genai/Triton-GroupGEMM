...
def main():
    ...
    for i in range(group_size):
        assert torch.allclose(ref_out[i], tri_out[i], atol=1e-3, rtol=1e-3)
    
    benchmark_batches_gbps.run(show_plots=True, print_data=True, save_path="group-gemm-performance-optim-pow2-pad")
    benchmark_batches_gflops.run(show_plots=True, print_data=True, save_path="group-gemm-performance-optim-pow2-pad")
